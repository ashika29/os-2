...
void RoundRobin(int c){ 
	...
	D:
	printf("\n\n Enter Time Quantum(even):\t");
	scanf("%d", &tq);
	if((tq%2)!=0){
		printf("\n Sorry, invalid Time quantum. Please enter an even TQ again.. \n");
		goto D;
	}
	printf("\n\tPID\t\tArrival Time\tRemaining Burst Time\tCompletion time\t Turnaround Time\t Waiting Time\n");
	for(total = 0, i = 0; x != 0; )
	{
		...
		{
			x--;
printf("\nP-%d\t\t%d\t\t%d\t\t%d\t\t %d\t\t\t %d",Q2[i].pid2,Q2[i].at,bursttime[i],total,total-arrivaltime[i],total-arrivaltime[i]-bursttime[i]);
